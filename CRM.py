...
def nuevo_cliente():
    def guardar():
        ...
        if not empresa.get():
            messagebox.showerror('Error', 'El campo empresa es obligatoria!')
            return
        cliente = {
            'nombre': nombre.get(),
...
def eliminar_cliente():
    ...
    respuesta = messagebox.askokcancel('Seguro?', 'Estas seguro de que quieres eliminar el cliente ' + cliente[1] + '?')
    if respuesta:
